...
SamplePlayer::actionImpl()
{
  // 基本的に無限ループでここが毎回呼ばれる．
    ...
    std::cout << "agent no. " << world().self().unum() << std::endl;
    // std::cout << "sample_player.cpp:275 role_ptr type is: " << typeid(*role_ptr).name() << std::endl;
...
SamplePlayer::handleActionEnd()
{
    ...
    dlog.addText( Logger::WORLD,
                  ...
                  diff.y );
    dlog.addText( Logger::WORLD,
                  "HOGEhOGe unum=%d pos=(%lf %lf) diff=(%lf %lf)",
                  world().self().unum(),
                  world().self().pos().x,
                  world().self().pos().y,
                  diff.x,
